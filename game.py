...
def exit_leads_to(exits, direction):
    ...
    'Reception'
    """
    #return exits[direction]    previous method
    #r = exits[direction]
    #print("/n THIS IS A TEST /n")
    #print(rooms[r]["name"])
    #return(rooms[r]["name"])
...
def print_menu(exits):
    ...
    for item in exits:
        print_menu_line(item, exit_leads_to(exits, item))
        # COMPLETE THIS PART:
...
def menu(exits):
    ...
    while True:
        ...
        print_menu(exits)
        # Read player's input
        nextR = input("Please enter the exit you want to take ")
        # Normalise the input
        # Check if the input makes sense (is valid exit)
        if is_valid_exit(exits, nextR) ==True:
            return nextR
            False
        else:
            True
